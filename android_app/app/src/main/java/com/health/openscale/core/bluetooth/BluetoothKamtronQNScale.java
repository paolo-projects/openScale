...
public class BluetoothKamtronQNScale extends BluetoothCommunication {
    ...
    public BluetoothKamtronQNScale(Context context) {
        super(context);
        this.mContext = context;
        bodyShape = mContext.getResources().getStringArray(R.array.bodyshape_values);
    }
                ...
                if (data[5] == (byte) 0) {
                    ...
                    if (!this.hasReceived) {
                        ...
                        if (weightKg > 0.0f) {
                            ...
                            // Using the original lib (libyolanda_calc) for better results.
                            // I gave a quick look at the asm code, the equations are somewhat obfuscated or heavily optimized
                            // and I'm not good enough to make a Java class from it. I didn't find any junk code logging data and
                            // sending it to unknown servers. The lib doesn't have many functions, just the needed ones to parse the impedance values
                            // into useful parameters.
                            // The logging code is in the Java part of the app, so using just the lib alone should be enough to avoid any of it
                            BleScaleData bleData = new BleScaleData();
                            bleData.setWeight(weightKg);
                            bleData.setResistance50(resistance1);
                            bleData.setResistance500(resistance2);

                            // method is a parameter that selects what kind of algorithm to use. values from 1 to 5 select other algorithms that are only using one impedance variable
                            // any other value, 0 included, will select the method that uses both the impedance measures
                            //
                            // athlete type is 0 for a normal athlete and 1 for someone who trains a lot
                            // I don't know what difference it makes, but I suppose it changes the way data is calculated
                            // Should probably include an option in the profile settings but I don't know if it's actually of any use
                            bleData.initWithAthlete(0, (int)scaleUser.getBodyHeight(), scaleUser.getAge(), scaleUser.getGender().isMale() ? 1 : 0, 0);

                            btScaleMeasurement.setFat((float)bleData.getBodyfat());// qnscalelib.getFat(weightKg, impedance));
                            btScaleMeasurement.setWater((float)bleData.getWater()); // qnscalelib.getWater(weightKg, impedance));
                            btScaleMeasurement.setMuscle((float)bleData.getMuscle()); // qnscalelib.getMuscle(weightKg, impedance));
                            btScaleMeasurement.setBone((float)bleData.getBone()); //qnscalelib.getBone(weightKg, impedance));
                            btScaleMeasurement.setVisceralFat(bleData.getVisfat());
                            btScaleMeasurement.setLbm((float)bleData.getLbm());
                            btScaleMeasurement.setSubFat((float)bleData.getSubfat());
                            btScaleMeasurement.setProtein((float)bleData.getProtein());
                            btScaleMeasurement.setMetabolicAge(bleData.getBodyAge());
                            btScaleMeasurement.setBodyShape(bodyShape[bleData.getBodyShape()]);
